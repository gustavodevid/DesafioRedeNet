...
export const addPokemonToPokedex = async (userId: number, pokemonCodeAPI: number) => {
...
export const removePokemonFromPokedex = async (userId: number, pokemonCodeAPI: number) => {
  const user = await prismaClient.user.findUnique({
    where: { id: userId },
    include: { pokedex: true },
  });
